...
int main(int argc, char *argv[]) {
  try {
    ...
    auto listen_endpoint = listen_results.begin()->endpoint();

    auto target_results = tcp::resolver(ctx).resolve(argv[3], argv[4]);
    auto target_endpoint = target_results.begin()->endpoint();
